...
load_datasets_for_size <- function(size) {
  ...
  synthpop_dir <- paste0("Heart Failure Prediction Data/Data/synthpop/m", size, "/")
  arf_dir <- paste0("Heart Failure Prediction Data/Data/arf/m", size, "/")
  privbayes_dir <- paste0("Heart Failure Prediction Data/Data/privbayes/m", size, "/")
  ctgan_dir <- paste0("Heart Failure Prediction Data/Data/ctgan/m", size, "/")
  
  # Load datasets based on size
  synthpop_datasets <- load_csv_files(synthpop_dir, paste0("^syn_heart_failure_synthpop_", size))
  arf_datasets <- load_csv_files(arf_dir, paste0("^syn_heart_failure_arf_", size))
  privbayes_datasets <- load_csv_files(privbayes_dir, paste0("^syn_heart_failure_privbayes_", size))
  ctgan_datasets <- load_csv_files(ctgan_dir, paste0("^syn_heart_failure_ctgan_", size))
...
process_and_plot <- function(method, m_values, base_path) {
  ...
  for (m in m_values) {
    ...
    dateien <- list.files(
      path = paste0("Heart Failure Prediction Data/Data/", base_path, "/"), 
      pattern = pattern, 
      full.names = TRUE
    )
    ...
    # Generate plots
    plot_result <- compare(syn_data, heart_failure, nrow = 6, ncol = 2)
    plot_output <- plot_result$plots
    
    # Save plot
    ggsave(
      filename = paste0("Heart Failure Prediction Data/evaluation_utility/fit_for_purpose_utility/plot_", method, "_", m, ".png"), 
      plot = plot_output, 
      height = 15, 
      width = 10
    )
...
generate_comparison_plot <- function(variable, datasets_m5_wo_original, datasets_m10_wo_original, datasets_m50_wo_original, heart_failure, output_path, m_values) {
  ...
  for (m in m_values) {
    ...
    plot_synthpop <- compare(synthpop_group, heart_failure, vars = variable)$plots
    plot_arf <- compare(arf_group, heart_failure, vars = variable)$plots
    plot_privbayes <- compare(privbayes_group, heart_failure, vars = variable)$plots
    plot_ctgan <- compare(ctgan_group, heart_failure, vars = variable)$plots
    plot_tvae <- compare(tvae_group, heart_failure, vars = variable)$plots
    plot_tabsyn <- compare(tabsyn_group, heart_failure, vars = variable)$plots
...
evaluate_model <- function(datasets, m, output_path) {
  ...
  coeff_results <- coeff_results %>%
    mutate(Method = case_when(
      grepl("syn_heart_failure_privbayes", Dataset) ~ "PrivBayes",
      grepl("syn_heart_failure_arf", Dataset) ~ "ARF",
      grepl("syn_heart_failure_synthpop", Dataset) ~ "Synthpop",
      grepl("syn_heart_failure_ctgan", Dataset) ~ "CTGAN",
      grepl("syn_heart_failure_tvae", Dataset) ~ "TVAE",
      grepl("syn_heart_failure_tabsyn", Dataset) ~ "TABSYN",
      Dataset == "heart_failure" ~ "Real",
      TRUE ~ "Unknown"
    ))
  ...
  regression_coeff_plot_combined <- ggplot(combined_results, aes(y = forcats::fct_rev(Term), x = combined_estimate, col = Method)) + 
    ...
    scale_y_discrete(
      labels = c(
        "Age" = "Age [years]",
        "ChestPainTypeATA" = "Chest pain type [ATA]",
        "ChestPainTypeNAP" = "Chest pain type [NAP]",
        "ChestPainTypeTA" = "Chest pain type [TA]",
        "Cholesterol" = "Serum cholesterol [mm/dl]",
        "ExerciseAnginaY" = "Exercise-induced angina [Yes]",
        "FastingBS1" = "Fasting blood sugar [> 120 mg/dl]",
        "MaxHR" = "Maximum heart rate achieved",
        "Oldpeak" = "oldpeak = ST",
        "RestingBP" = "Resting blood pressure [mm Hg]",
        "RestingECGNormal" = "Resting electrocardiogram results [Normal]",
        "RestingECGST" = "Resting electrocardiogram results [ST]",
        "SexM" = "Sex [Male]",
        "ST_SlopeFlat" = "Slope of the peak exercise ST segment [Flat]",
        "ST_SlopeUp" = "Slope of the peak exercise ST segment [Up]"
      )) +
  ...
  ggsave(output_path, plot = regression_coeff_plot_combined, width = 8, height = 14)
}
...
compute_s_pMSE <- function(method, m) {
  ...
  # Compute mean S_pMSE
  compare_results <- lapply(syn_data, compare, heart_failure)
...
evaluate_regression_per_dataset <- function(method, m, real_test, syn_data_list) {
  ...
  for (syn_data in syn_data_list) {
    ...
    #syn_data <- read.csv("IST-3 Data/Data/train_data/synthpop/syn_real_train_synthpop_1.csv")
    #target_var <- "outcome"
...
process_dataset <- function(df) {
  df %>%
    mutate(
      ...
      FastingBS = as.integer(FastingBS)
    ) %>%
    dummy_cols(select_columns = c("ChestPainType", "RestingECG", "ST_Slope"),
